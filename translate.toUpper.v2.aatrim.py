...
def translate(pack):
    """
    Translate a DNA sequence into its protein equivalent, stopping at the first stop codon.
    Stop codon is not included in the output. Ns translate to X and set the bad flag.
    """
    codon_table = {
    ...
    # walk codons, stop at first stop, exclude the stop itself
    for i in range(0, len(seq) - 2, 3):
        ...
        if codon in stops:
            break
        if 'N' in codon:
            aminos_list.append('X')
            bad = True
        else:
            # If a non-ACGT sneaks in, default to X and mark bad
            aa = codon_table.get(codon, 'X')
            if aa == 'X':
                bad = True
            aminos_list.append(aa)

    aminos = ''.join(aminos_list)
    return (var, aminos, bad)
...
if __name__ == '__main__':
    ...
    raw_prots = pool.imap_unordered(translate, fasta_reader(args.infile),
                                    chunksize=10000)
    # set up writers
    bad_flag = False
    ...
    for header, prot, bad_prot in raw_prots:
        # Exclude zero-length translations (start with stop codon)
        if len(prot) == 0:
            continue
